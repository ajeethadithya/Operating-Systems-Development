...
/* emulate 386 "pushal" instruction */
	...
	*pushsp = pptr->pesp = (unsigned long)saddr;
	...
	//kprintf("\nfrm number:%d",frm_number);
	frm_tab[frm_number].fr_status = FRM_MAPPED;
	frm_tab[frm_number].fr_pid = pid;
	frm_tab[frm_number].fr_type = FR_DIR;
	proctab[pid].pdbr = NBPG * (FRAME0 + frm_number);
	pd_entry = NBPG * (FRAME0 + frm_number);
	for(i = 0; i < NFRAMES; i++) {
		j = i;
		pd_entry[i].pd_write = 1;
	}
	        //initializing the first 4 PDs
	i = 0;
	while(i <= 3) {
